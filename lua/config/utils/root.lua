local fn = require("config.utils.fn")
local fs = require("config.utils.path")
...
M = {
	...
	pattern = function(buf, patterns)
		local result = vim.fs.find(function(path)
			return list.any(function(pattern)
				return fs.match(pattern, path)
			end, patterns)
		end, {
	...
	---@return string?
	detect = function(buf)
		return M.lsp(buf) or M.pattern(buf, vim.g.root_pattern) or M.parent(buf) or vim.uv.cwd()
	end,
